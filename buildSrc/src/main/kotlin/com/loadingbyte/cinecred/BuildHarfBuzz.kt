...
abstract class BuildHarfBuzz : DefaultTask() {
    ...
    fun run() {
        ...
        val srcFile = repositoryDir.get().asFile.resolve("src/harfbuzz-subset.cc")
        val outFile = outputFile.get().asFile

        // Side note 1: We don't use `HB_LEAN` since that disables a lot of advanced shaping functionality (like
        // automatic fractions) which we actually wish to support.
        //
        // Side note 2: The following macros could be enabled on macOS and Linux, however, they are only needed in case
        // we create blobs from files, which we don't do at the moment. So for the time being, we omit these options to
        // maximize portability.
        //     -DGETPAGESIZE -DHAVE_MPROTECT -DHAVE_SYSCONF -DHAVE_SYS_MMAN_H -DHAVE_UNISTD_H
        val macros = listOf("HB_DISABLE_DEPRECATED")
        ...
        if (forPlatform.os == WINDOWS) {
            ...
            cmd += listOf("-std=c++14", "-O2", "-fPIC", "-flto", "-fno-rtti", "-fno-exceptions")
