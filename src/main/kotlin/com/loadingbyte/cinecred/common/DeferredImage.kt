...
class DeferredImage(var width: Float = 0f, var height: Float = 0f) {
            ...
            for (insn in insns) {
                val newInsn = when (insn) {
                    ...
                    is Instruction.DrawString -> Instruction.DrawString(
                        ...
                        scaling * insn.justificationWidth, scaling * insn.scaling
                    )
    ...
    fun drawShape(
        ...
        addInstruction(layer, Instruction.DrawShape(color, shape, x, y, scaling, fill))
    }
    ...
    fun drawString(
        ...
        if (!layer.isHelperLayer) {
            ...
            height = max(height, y + scaling * font.style.heightPx)
        }
        addInstruction(layer, Instruction.DrawString(font, str, x, y, justificationWidth, scaling))
    }
    ...
    fun materialize(g2: Graphics2D, layers: List<Layer>) {
        for (layer in layers)
            for (insn in instructions[layer]!!)
    ...
    private fun materializeInstruction(g2: Graphics2D, insn: Instruction) {
        when (insn) {
            ...
            is Instruction.DrawString -> {
                ...
                g2.withNewG2 { g2 ->
                    g2.color = insn.font.style.foreground
